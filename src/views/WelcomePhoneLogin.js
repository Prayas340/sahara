...
export function renderWelcomePhoneLogin(onNavigate) {
          ...
          <div class="lg:col-span-7 bg-white p-5 sm:p-6 lg:p-7 xl:p-8 flex flex-col justify-between gap-3.5 sm:gap-4">
            ...
            <div class="flex flex-col gap-2.5 pt-0.5">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                <button 
                  class="w-full h-11 sm:h-12 rounded-2xl bg-white hover:bg-gray-50 border-2 border-gray-200 hover:border-gray-300 text-gray-700 font-bold text-xs sm:text-sm transition-all flex items-center justify-center gap-2 px-3 shadow-sm active:scale-[0.99] cursor-pointer" 
                  id="login-google-btn" 
                  type="button"
                >
                  ...
                  <span id="welcome-google-text">${t.signInWithGoogle || 'Sign in with Google'}</span>
                </button>

                <button 
                  class="w-full h-11 sm:h-12 rounded-2xl bg-[#006e1c] hover:bg-[#0d631b] text-white font-bold text-xs sm:text-sm transition-all flex items-center justify-center gap-2 px-3 shadow-md active:scale-[0.99] cursor-pointer" 
                  id="signup-google-btn" 
                  type="button"
                >
                  <svg class="w-4 h-4 shrink-0 bg-white p-0.5 rounded-full" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"/>
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"/>
  ...
  setTimeout(() => {
    ...
    // Google Sign In handler (only fetches details if account already exists)
    googleBtn?.addEventListener('click', async () => {
      googleBtn.disabled = true;
      googleBtn.innerText = 'Signing in...';
      const res = await authService.signInWithGoogle('elder', 'signin');
      ...
      if (res?.success) {
        if (res.elderProfile && res.elderProfile.name) {
          ...
          showToast('No existing account found. Please click "Sign up with Google" to create a new companion profile.', 'error');
        }
    ...
    // Google Sign Up handler (creates new account and redirects to Step 3 profile-setup)
    ...
    signupGoogleBtn?.addEventListener('click', async () => {
      ...
      signupGoogleBtn.innerText = 'Creating account...';
      const res = await authService.signInWithGoogle('elder', 'signup');
      signupGoogleBtn.disabled = false;
      signupGoogleBtn.innerHTML = `
        <svg class="w-4 h-4 shrink-0 bg-white p-0.5 rounded-full" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"/></svg>
        <span>Sign up with Google</span>
      `;
      ...
      if (res?.success) {
        ...
        showToast(res?.message || 'Unable to create account with Google.', 'error');
      }
    });
