            ...
            <div class="space-y-6">
              ...
              ${(() => {
                  ...
                  <div class="card-tactile bg-white p-5 rounded-2xl border border-[#cdf2cb] shadow-sm flex flex-col justify-between">
                    ...
                    <div>
                      <div class="flex items-baseline gap-1.5">
                        ...
                        <span class="text-sm font-bold text-[#0d631b]">Points</span>
                      </div>
                  ...
                  <div class="flex items-center justify-between flex-wrap gap-2">
                    <div class="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p class="text-xs sm:text-sm text-[#40493d]">Daily scores tracking cognitive engagement and consistency (max 250 pts / 5 sessions per day).</p>
                      </div>
                    </div>
                    <div class="flex items-center gap-4 text-xs font-bold text-[#40493d] flex-wrap">
                      <div class="flex items-center gap-1.5">
                        <span class="w-3 h-3 rounded-full bg-gradient-to-r from-[#0d631b] to-[#43a047]"></span>
                        <span>High Score (200-250 pts / 4-5 Sessions)</span>
                      </div>
                      <div class="flex items-center gap-1.5">
                        <span class="w-3 h-3 rounded-full bg-gradient-to-r from-[#81c784] to-[#a3f69c]"></span>
                        <span>Moderate (50-150 pts)</span>
                      </div>
                      <div class="flex items-center gap-1.5">
                        <span class="w-3 h-3 rounded-full bg-emerald-100 border border-[#cdf2cb]"></span>
                        <span>No Sessions (0p)</span>
                      </div>
                    </div>
                  </div>
                    ...
                    ${(ga.last7Days && ga.last7Days.length > 0 ? ga.last7Days : []).map((d, i) => {
                      ...
                      <div class="flex flex-col items-center gap-2 h-full justify-end">
                        ...
                        <div class="w-full max-w-[48px] bg-white rounded-t-xl overflow-hidden flex flex-col justify-end p-0.5 h-32 border border-[#cdf2cb]">
                          <div style="height: ${heightPercent}%;" class="w-full rounded-t-lg ${score >= 200 ? 'bg-gradient-to-t from-[#0d631b] to-[#43a047]' : score > 0 ? 'bg-gradient-to-t from-[#81c784] to-[#a3f69c]' : 'bg-emerald-50/50'}"></div>
                        </div>
                ...
                <div class="card-tactile bg-white rounded-3xl p-6 sm:p-8 shadow-md border border-[#cdf2cb] space-y-4">
                  <div class="flex items-center justify-between flex-wrap gap-2">
                    <div class="flex items-center gap-2">
                      <span class="material-symbols-outlined text-2xl text-[#0d631b]">history</span>
                      <div>
                        <h3 class="text-xl font-extrabold text-[#032109]">Recent Game Sessions & Score Logs</h3>
                        <p class="text-xs sm:text-sm text-[#40493d]">Detailed audit trail of each 1-minute session completed by ${patient.name ? patient.name.split(' ')[0] : 'Patient'}.</p>
                      </div>
                    </div>
                    <span class="text-xs font-bold text-[#0d631b] bg-[#d9fdd6] px-3 py-1 rounded-full border border-[#cdf2cb]">
                      ${(ga.sessions && Array.isArray(ga.sessions) ? ga.sessions.length : (ga.recentScores ? ga.recentScores.length : 0))} Total Sessions Logged
                    </span>
                  </div>

                  <div class="overflow-x-auto pt-2">
                    ${(!ga.sessions || ga.sessions.length === 0) && (!ga.recentScores || ga.recentScores.length === 0) ? `
                      <div class="text-center py-10 px-4 bg-[#ebffe7]/40 rounded-2xl border border-dashed border-[#cdf2cb]">
                        <span class="material-symbols-outlined text-4xl text-[#0d631b] mb-2">sports_esports</span>
                        <h4 class="text-base font-bold text-[#032109]">No Game Sessions Recorded Yet</h4>
                        <p class="text-xs text-[#40493d] mt-1 max-w-sm mx-auto">
                          Scores and memory recall accuracy will appear here in real-time as ${patient.name ? patient.name.split(' ')[0] : 'the elder'} plays 1-minute memory games.
                        </p>
                      </div>
                    ` : `
                      <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-[#ebffe7] text-sm font-medium text-[#032109]">
                          ${(ga.sessions && ga.sessions.length > 0 ? ga.sessions : (ga.recentScores || [])).map((sess, sIdx) => {
                            ...
                            return `
                              <tr class="hover:bg-[#ebffe7]/50 transition-colors">
                                ...
                                <td class="py-3.5 px-3 text-center font-bold text-xs">
                                  ${sess.remainingTimeSeconds !== undefined && sess.remainingTimeSeconds > 0
                                    ? `<span class="text-emerald-700 font-extrabold">${sess.remainingTimeSeconds}s</span>`
                                    : `<span class="text-amber-700">0s (expired)</span>`}
                                </td>
                                <td class="py-3.5 px-3 text-center">
                                  <span class="px-2.5 py-0.5 rounded-full text-xs font-extrabold ${isTimedOutSess ? 'bg-amber-100 text-amber-900' : 'bg-[#cdf2cb] text-[#006e1c]'}">
                                    ${sess.accuracy !== undefined ? sess.accuracy : (isTimedOutSess ? 0 : 100)}%
                                  </span>
                                </td>
                                <td class="py-3.5 px-3 text-right">
                                  <span class="text-base font-extrabold ${ptsEarned > 0 ? 'text-[#0d631b]' : 'text-gray-400'}">
